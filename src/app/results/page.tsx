    ...
    <nav style={{
      ...
      {showRetake && (
        <Link href="/quiz" style={{
          ...
          <RotateCcw size={13} /> Take the Quiz
        </Link>
      )}
    </nav>
  );
...
function LoadingScreen() {
  return (
    <div style={{ background: "#ffffff", minHeight: "100vh", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "80vh", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 32, marginBottom: 16 }}>⏳</div>
          <p style={{ color: "#6b7280", fontSize: 15 }}>Loading shared results...</p>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ background: "#ffffff", minHeight: "100vh", color: "#111720", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "80vh", padding: "24px", position: "relative", zIndex: 1 }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <h2 style={{ color: "#111720", fontSize: 24, fontWeight: 800, marginBottom: 12 }}>No results found</h2>
          <p style={{ color: "#6b7280", marginBottom: 24 }}>
            This link may have expired or the results could not be found. Take the quiz to get your own matched roles.
          </p>
          <Link href="/quiz" style={{
...
// ── SHARED LINK FETCHER ────────────────────────────────────────
// Uses a ref to fire the fetch exactly once without useEffect.

function SharedResults({ id }: { id: string }) {
  const [data, setData] = useState<{ answers: UserAnswers; matches: MatchedRole[] } | "loading" | "error">("loading");
  ...
  if (data === "error") return <NoResultsScreen />;
  return <ResultsDisplay answers={data.answers} matches={data.matches} isShared={true} />;
}
...
export default function ResultsPage() {
  ...
  if (id) return <SharedResults id={id} />;

  const raw = sessionStorage.getItem("quizAnswers");
  if (!raw) return <NoResultsScreen />;

  const parsedAnswers: UserAnswers = JSON.parse(raw);
  const computedMatches = runMatchingEngine(parsedAnswers);
  return <ResultsDisplay answers={parsedAnswers} matches={computedMatches} isShared={false} />;
}
    ...
    <div style={{ background: "#ffffff", minHeight: "100vh", color: "#111720", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 720, margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        <div style={{
          ...
          <div style={{ position: "absolute", top: -40, right: -40, width: 200, height: 200, background: "radial-gradient(circle, rgba(14,165,233,0.07), transparent 70%)", pointerEvents: "none" }} />
          ...
          {topMatch && (
            <p style={{ fontSize: 15, color: "#6b7280", maxWidth: 480, margin: "0 auto 20px" }}>
              ...
              {isShared && " Want to see your own results?"}
            </p>
          )}
          ...
          {isShared ? (
            <Link href="/quiz" style={{
              ...
              Find my own matches <ArrowRight size={14} />
            </Link>
          ) : (
            matches.length > 0 && (
              <>
                ...
                >
                  {copied
                    ...
                    : sharing
                    ? "Generating link..."
                    : <><Share2 size={15} /> Share my results</>}
                </button>
                {shareId && !copied && (
                  <div style={{ marginTop: 12, fontSize: 12, color: "#6b7280", padding: "8px 14px", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8 }}>
              ...
              <div key={role.id} style={{
                ...
                <div style={{ padding: "20px 24px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 200 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 100, background: `rgba(${pillar.rgb},0.1)`, color: pillar.color, border: `1px solid rgba(${pillar.rgb},0.2)` }}>
                            {pillar.label}
                          </span>
                    ...
                    <div style={{ textAlign: "center", flexShrink: 0 }}>
                      <div style={{ fontSize: 32, fontWeight: 800, color: scoreColor, lineHeight: 1 }}>{matchPercent}%</div>
                  ...
                  <div style={{ height: 4, background: "#e5e7eb", borderRadius: 100, marginTop: 16, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${matchPercent}%`, background: `linear-gradient(to right, ${scoreColor}, ${scoreColor}aa)`, borderRadius: 100 }} />
                  </div>

                  <button
                    onClick={() => setExpandedId(isExpanded ? null : role.id)}
                    style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 14, padding: "6px 0", background: "transparent", border: "none", color: "#6b7280", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                  >
                    {isExpanded
                ...
                {isExpanded && (
                  <div style={{ borderTop: "1px solid #e5e7eb", padding: "24px", background: "#ffffff" }}>
                    <p style={{ fontSize: 14, color: "#6b7280", lineHeight: 1.7, marginBottom: 24 }}>{role.description}</p>

                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20 }}>
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#10b981", marginBottom: 10 }}>✅ Skills you have</div>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {role.skillsRequired.filter(s => answers.skillsInUse.includes(s)).map(s => (
                            <span key={s} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 100, background: "rgba(16,185,129,0.08)", color: "#10b981", border: "1px solid rgba(16,185,129,0.2)" }}>{s}</span>
                          ))}
                      ...
                      {nearSkills.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#f59e0b", marginBottom: 10 }}>📖 Currently studying</div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                            {nearSkills.map(s => (
                              <span key={s} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 100, background: "rgba(245,158,11,0.08)", color: "#f59e0b", border: "1px solid rgba(245,158,11,0.2)" }}>{s}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {gapSkills.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#f43f5e", marginBottom: 10 }}>🎯 Skills to develop</div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                            {gapSkills.map(s => (
                              <span key={s} style={{ fontSize: 12, padding: "3px 10px", borderRadius: 100, background: "rgba(244,63,94,0.06)", color: "#f43f5e", border: "1px solid rgba(244,63,94,0.2)" }}>{s}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {role.certifications.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "#8b5cf6", marginBottom: 10 }}>🏆 Recommended certifications</div>
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            {role.certifications.map(cert => (
                              <div key={cert} style={{ fontSize: 12, padding: "6px 12px", borderRadius: 8, background: "rgba(139,92,246,0.06)", color: "#7c3aed", border: "1px solid rgba(139,92,246,0.2)" }}>{cert}</div>
                            ))}
                          </div>
                        </div>
                      )}
                    ...
                    <div style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid #e5e7eb", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 12, padding: "4px 12px", borderRadius: 6, fontWeight: 600, background: role.level === "entry" ? "rgba(16,185,129,0.08)" : "rgba(245,158,11,0.08)", color: role.level === "entry" ? "#10b981" : "#f59e0b", border: `1px solid ${role.level === "entry" ? "rgba(16,185,129,0.2)" : "rgba(245,158,11,0.2)"}` }}>
        ...
        {matches.length > 0 && (
          <div style={{ marginTop: 40, textAlign: "center", padding: "32px", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 12 }}>
            <p style={{ color: "#6b7280", fontSize: 14, marginBottom: 16 }}>
              {isShared
                ? "Want to find your own matched roles?"
                : "Not seeing what you expected? Try updating your skills or changing your preferences."}
            </p>
            <Link
              href="/quiz"
              style={{ display: "inline-flex", alignItems: "center", gap: 8, background: isShared ? "#0ea5e9" : "transparent", color: isShared ? "#ffffff" : "#111720", fontWeight: 700, fontSize: 14, padding: "10px 20px", borderRadius: 8, border: isShared ? "none" : "1px solid #e5e7eb", textDecoration: "none" }}
            >
              {isShared ? <>Take the Quiz <ArrowRight size={14} /></> : <><RotateCcw size={14} /> Retake the Quiz</>}
            </Link>
          </div>
        )}
      </div>
